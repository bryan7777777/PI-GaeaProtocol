...
// Se foi enviado um avatar predefinido
if (!empty($_POST['predefined_avatar'])) {
    $avatarId = htmlspecialchars($_POST['predefined_avatar']);
    
    // Valida se é um ID de avatar válido
    $validAvatars = ['avatar_1', 'avatar_2', 'avatar_3', 'avatar_4', 'avatar_5', 
                    'avatar_6', 'avatar_7', 'avatar_8', 'avatar_9', 'avatar_10', 
                    'avatar_11', 'avatar_12'];
    
    if (!in_array($avatarId, $validAvatars)) {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE usuarios SET Avatar = ? WHERE idUsuario = ?");
        $stmt->execute(["predefined://" . $avatarId, $userId]);
        
        // Atualiza sessão
        $_SESSION['user']['fotoPerfil'] = "predefined://" . $avatarId;
        
        echo json_encode(['success' => true, 'message' => 'Avatar definido com sucesso', 'avatar' => "predefined://" . $avatarId]);
    } catch (PDOException $e) {
        http_response_code(500);
        echo json_encode(['error' => 'Erro ao atualizar banco de dados']);
        error_log('Erro ao definir avatar: ' . $e->getMessage());
    }
    exit;
}

// Se foi enviado um ícone padrão (Font Awesome)
